            ...
            {receipts.slice(0, 8).map((r) => (
              <li key={r.id} data-testid={CRMC.planReceiptRow}
                ...
                <span className="flex items-center gap-2">
                  ...
                  <button type="button" data-testid="plan-receipt-pdf" data-receipt-id={r.id}
...
function AllinCostsSummary({ contract, costs, customerId, onChanged }) {
  ...
  return (
    <div data-testid={CRMC.planAllin} className="space-y-2 rounded-xl border bg-card p-4 shadow-[var(--shadow-card)]">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="section-title">Biaya all-in · {contract.unit_code || "Unit"}</p>
          <p className="text-[12px] text-muted-foreground">
            {(contract.costs || {}).scheme_name || "komponen biaya kontrak"} · {comps.length} komponen
          </p>
        </div>
        <Link to={`/customers/${customerId}?tab=kontrak53`} data-testid={CRMC.planAllinOpen}
          className="inline-flex items-center gap-1 text-[12px] font-medium text-primary hover:underline">
          Buka di Kontrak &amp; Legal <ExternalLink className="h-3 w-3" />
        </Link>
      </div>
      <p className="flex items-start gap-1.5 rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-2 text-[12px] text-sky-900">
        <Info className="mt-0.5 h-4 w-4 shrink-0" />
        <span>
          Biaya ini <b>tidak masuk Nilai kontrak / piutang unit</b> di atas: uangnya bukan pendapatan pengembang,
          melainkan <b>titipan pembeli</b> yang disalurkan ke pihak ketiga (BPHTB, notaris, dsb.). Karena itu ditagih
          lewat <b>Invoice Biaya (INB)</b> dan diterima lewat <b>kuitansi KWB</b> — buku terpisah dari AR, dan pencairan KPR
          hanya melunasi piutang unit.
        </span>
      </p>
      <div className="grid gap-2 sm:grid-cols-4">
        {[["Ditanggung pembeli", passThrough], ["Sudah ditagih (INB)", costs.invoiced],
          ["Sudah diterima (KWB)", costs.received], ["Saldo titipan", costs.titipan_balance]].map(([label, val]) => (
          <div key={label} className="rounded-lg border bg-secondary/40 px-2.5 py-1.5">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">{label}</p>
      ...
      <ul className="divide-y rounded-lg border text-[12px]">
        {comps.map((c) => (
          <li key={c.code} data-testid={CRMC.planAllinRow} className="flex items-center justify-between px-2.5 py-1.5">
            <span>{c.name || c.code}
              <span className="ml-1 text-muted-foreground">· {c.treatment === "developer_borne" ? "ditanggung pengembang" : "titipan pembeli"}</span>
            </span>
            <span className="tabular-nums"><MoneyText value={c.amount} /></span>
          </li>
        ))}
      </ul>
      {devBorne ? (
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const plans = await Promise.all(deals.map(async (deal) => {
        ...
        const [ar, late, contract] = await Promise.all([
          api.get(`/finance/ar/${deal.id}`).then((r) => r.data).catch(() => null),
          api.get(`/finance/late-fees/${deal.id}`).then((r) => r.data.data).catch(() => null),
          api.get("/contracts", { params: { deal_id: deal.id, limit: 5 } })
            .then((r) => (r.data.data || []).find((c) => c.deal_id === deal.id) || null).catch(() => null),
        ]);
        // Biaya all-in (BPHTB, notaris, dll.) = TITIPAN pembeli, bukan piutang unit → buku terpisah.
        const costs = contract
          ? await api.get(`/contracts/${contract.id}/costs-ledger`).then((r) => r.data.data).catch(() => null)
          : null;
        return { deal, plan: ar, late, contract, costs };
      }));
      setRows(plans);
    } catch (e) {
