...
def validate_scheme(payload: dict) -> dict:
    ...
    pct = sum(float(t.get("pct") or 0) for t in tr if not int(t.get("amount") or 0))
    has_pct = any(not int(t.get("amount") or 0) for t in tr)
    if has_pct and abs(pct - 100) > 0.01:
        raise ValueError(f"Total persentase tahap ber-% harus 100% dari sisa plafon setelah tahap bernominal "
                         f"(sekarang {pct:g}%).")
    codes = set()
    for t in tr:
...
def _norm_bank(v) -> str:
    return " ".join(str(v or "").lower().replace("bank ", " ").split())


def bank_compatible(scheme_bank, app_bank) -> bool:
    """Skema 'semua bank' cocok ke siapa saja; skema ber-bank cocok bila namanya sama atau
    salah satu memuat yang lain (mis. pengajuan 'BTN' ↔ skema 'BTN KC Bandung')."""
    if not scheme_bank:
        return True
    if not app_bank:
        return False
    a, b = _norm_bank(scheme_bank), _norm_bank(app_bank)
    return a == b or a.startswith(b) or b.startswith(a) or f" {b} " in f" {a} " or f" {a} " in f" {b} "


async def schemes_for_bank(org: str, bank: str = None) -> list:
    """Skema aktif yang boleh dipakai pengajuan bank tertentu (khusus bank dulu, lalu semua bank)."""
    await ensure_default(org)
    rows = await db.kpr_disbursement_schemes.find({"org_id": org, "is_active": True}, {"_id": 0}).to_list(200)
    rows = [r for r in rows if bank_compatible(r.get("bank"), bank)]
    return sorted(rows, key=lambda r: (0 if r.get("bank") else 1, r.get("name") or ""))
...
def build_tranches(scheme: dict, plafon: int) -> list:
    """Tahapan kontrak dari skema + plafon SP3K. Tahap bernominal = tetap (retensi); tahap ber-%
    membagi SISA (plafon − Σ nominal). Bila semua tahap bernominal dan jumlahnya ≠ plafon, tahap
    pertama menyerap selisih supaya total tahapan selalu = plafon."""
    tr = scheme.get("tranches") or []
    plafon = int(plafon)
    fixed = sum(int(t.get("amount") or 0) for t in tr)
    pct_idx = [i for i, t in enumerate(tr) if not int(t.get("amount") or 0)]
    base = max(plafon - fixed, 0)
    out, acc = [], 0
    for i, t in enumerate(tr):
        if i in pct_idx:
            amt = base - acc if i == pct_idx[-1] else int(round(base * float(t.get("pct") or 0) / 100))
            acc += amt
        else:
            amt = int(t["amount"])
        out.append({"code": t["code"], "name": t["name"], "pct": t.get("pct"), "amount": amt,
                    "condition": t["condition"], "status": "open", "disbursement_id": None})
    if out and not pct_idx and fixed != plafon:
        out[0]["amount"] = max(plafon - (fixed - int(tr[0].get("amount") or 0)), 0)
        out[0]["adjusted"] = True
    return out
...
async def assign_scheme(org: str, contract: dict, app: dict, scheme_id: str, actor: str,
    ...
    if not bank_compatible(sch.get("bank"), app.get("bank_name")) and app.get("bank_name"):
...
async def default_scheme(org: str, bank: str = None) -> dict:
    """Skema bawaan untuk pengajuan bank tertentu (Pusat Konfigurasi › Pencairan KPR):
    1) satu-satunya skema khusus bank yang cocok → dipakai;
    2) beberapa skema khusus bank → None (harus dipilih manual, tidak menebak);
    3) tanpa skema khusus → `[CFG] kpr.disbursement.default_scheme_code` bila cocok dengan bank
       (skema 'semua bank' atau bank sama), lalu satu-satunya skema 'semua bank' aktif."""
    import settings_store as cfg
    rows = await schemes_for_bank(org, bank)
    specific = [r for r in rows if r.get("bank")]
    if bank and len(specific) == 1:
        return specific[0]
    if bank and len(specific) > 1:
        return None
    code = str(await cfg.get("kpr.disbursement.default_scheme_code", org_id=org) or DEFAULT_SCHEME["code"])
    by_code = next((r for r in rows if (r.get("code") or "").upper() == code.upper()), None)
    if by_code:
        return by_code
    generic = [r for r in rows if not r.get("bank")]
...
async def ensure_scheme(org: str, contract: dict, app: dict, actor: str) -> dict:
    ...
    if app.get("tranches") or int(app.get("approved_plafon") or app.get("plafon") or 0) <= 0:
        return app
    sch = await default_scheme(org, app.get("bank_name"))
    if not sch:
        raise ValueError(f"Ada lebih dari satu / belum ada skema pencairan yang cocok untuk bank "
                         f"{app.get('bank_name') or '-'} — pilih skema pencairan secara manual.")
