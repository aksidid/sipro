...
async def build_breakdown(org: str, contract: dict) -> dict:
    ...
    if promo is None:
        # Potongan HARGA (skema diskon/promo/kupon bersasaran harga, DP, booking fee) dari snapshot
        # mesin harga; potongan komponen biaya dicatat terpisah pada baris COST_DISCOUNT.
        if pricing.get("discount_amount") is not None:
            promo = int(pricing.get("discount_amount") or 0)
        else:
    ...
    disc_lines = [{"code": x.get("code"), "name": x.get("name"), "source": x.get("source_label"),
                  ...
                  for x in (pricing.get("discount_lines") or [])
                  if int(x.get("amount") or 0)]
    price_lines = [x for x in disc_lines if x["target"] != "cost"]
    add("PROMO_DISCOUNT", "Promo / potongan harga", promo, group="potongan",
        treatment="discount", state="filled" if promo is not None else "empty",
        meta={"items": price_lines} if price_lines else None)
        ...
        for comp in components:
            ...
            add(comp["code"], comp["name"], comp.get("amount"), group="biaya",
                ...
                meta={"formula": comp.get("formula"), "source": comp.get("source"),
                      ...
                      "gross": int(comp.get("amount") or 0) + int(comp.get("discount") or 0)})
        cost_cut = sum(int(c.get("discount") or 0) for c in components
                       if not (c.get("kpr_only") and scheme != "kpr"))
        if cost_cut:
            cut_lines = [x for x in disc_lines if x["target"] == "cost"]
            add("COST_DISCOUNT", "Potongan biaya transaksi (promo all-in)", cost_cut,
                group="potongan_biaya", treatment="discount",
                note="Sudah dikurangkan pada komponen biaya di atas — nilai komponen yang tampil adalah bersih.",
                meta={"items": cut_lines} if cut_lines else None)
    ...
    return {
        ...
        "cost_discount": int(amt("COST_DISCOUNT") or 0),
        "total_discount": int(amt("PROMO_DISCOUNT") or 0) + int(amt("COST_DISCOUNT") or 0),
        "addon_total": sum(int(amt(c) or 0) for c in ("ADDON_SPEC", "EXCESS_LAND", "HOOK_FEE")),
        "nett_price": nett,
...
async def ensure_contract(org: str, deal: dict, customer: dict, scheme: str,
    ...
    # Skema pembayaran yang dipilih sales saat reservasi IKUT ke kontrak: `resolve_for_contract`
    # membacanya lebih dulu, sehingga aktivasi kontrak menagih skema yang sama dengan booking.
    psched = (await db.payment_schemes.find_one({"id": deal["scheme_id"], "org_id": org}, {"_id": 0})
              if deal.get("scheme_id") else None) or {}
    if psched and (psched.get("kind") or psched.get("type")) != scheme:
        psched = {}
    doc = {
        ...
        "payment_scheme_id": psched.get("id"), "payment_scheme_name": psched.get("name"),
    ...
    # Piutang BIAYA per komponen (BPHTB, notaris, …, sudah bersih potongan) tercatat di Finance
    # begitu kontrak lahir — bukan menunggu klik manual, supaya sinkron dengan piutang unit.
    import allin_engine as ae
    await ae.link_cost_docs_to_contract(org, doc)
    try:
        await ae.issue_cost_invoice(org, doc, actor)
    except ValueError:
        pass  # tidak ada komponen yang ditagih ke pembeli (all-in developer) atau sudah terbit
    return doc
...
async def legal_gates(org: str, contract: dict, breakdown: dict = None) -> dict:
    ...
    if scheme != "kpr":
        ...
        if bd.get("has_excess_land"):
            if spkt_wajib and not await _spkt_exists(org, contract):
    ...
    if scheme == "kpr":
        ...
        # Gerbang SPKT per jenis skema (`[CFG] addon.spkt_scheme_kinds`): pada skema tunai
        # tidak ada akad kredit, jadi titik wajibnya adalah AJB.
        if spkt_wajib and not await _spkt_exists(org, contract):
            b.append(("spkt_belum_ada", "Ada kelebihan tanah tetapi SPKT belum diterbitkan."))
    gate("ajb", b)
