...
async def recognize_revenue(deal: dict, org_id=ORG_ID, cogs=None, actor="system") -> dict:
    ...
    Pendapatan = total tagihan AR (harga bersih unit + add-on) — bukan `deal.price` mentah,
    karena kewajiban kontrak dibangun dari AR; bila keduanya beda, jurnal pengakuan tidak
    seimbang dan gagal terposting. HPP dari RAB per unit (lihat `revrec_cogs`).
    """
    deal_id = deal["id"]
    existing = await db.revenue_recognitions.find_one({"org_id": org_id, "deal_id": deal_id}, {"_id": 0})
    if existing:
        return existing
    inv = await db.ar_invoices.find_one({"org_id": org_id, "deal_id": deal_id}, {"_id": 0, "total": 1})
    price = int((inv or {}).get("total") or deal.get("price") or 0)
    import revrec_cogs as rc
    cinfo = {"cogs": int(cogs), "cogs_source": "manual", "rab_hpp": None} if cogs is not None \
        else await rc.unit_cogs(org_id, deal)
    cogs = cinfo["cogs"]
    ...
    doc = {
        ...
        "cogs_source": cinfo["cogs_source"], "rab_hpp": cinfo.get("rab_hpp"),
...
async def ap_aging(org_id=ORG_ID) -> dict:
    """Umur utang usaha: hanya tagihan yang SUDAH disetujui (= saldo 2-1100 di buku besar).

    Tagihan `pending_approval` belum menjadi utang — dilaporkan terpisah (`pending_value`)
    supaya angka AP outstanding di dasbor sama dengan Utang Usaha di neraca.
    """
    bills = await db.ap_invoices.find({"org_id": org_id}, {"_id": 0}).to_list(2000)
    approved = [b for b in bills if b.get("status") in ("approved", "partial", "paid")]
    entries = [(b.get("due_date"), int(b.get("net", 0)) - int(b.get("paid", 0)))
               for b in approved if b.get("status") != "paid"]
    agg = bucketize(entries)
    retention_held = sum(int(b.get("retention_held", 0) or 0) for b in approved
                         if not b.get("retention_released"))
    pending = [b for b in bills if b.get("status") == "pending_approval"]
    return {**agg, "retention_held": retention_held,
            "pending_value": sum(int(b.get("net", 0)) for b in pending),
...
async def finance_summary(org_id=ORG_ID) -> dict:
    ...
    return {
        ...
        "ap_pending_value": ap["pending_value"],
